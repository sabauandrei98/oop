...
#define MAX_MENU_INDEX 11
#define EXIT_MENU_INDEX 11
...
void printMenu()
{
	...
	printf("8. For a given medication name, see all medications, sorted ascending by concentration.\n");
	printf("9. Undo\n");
	printf("10. Redo\n");
	printf("11. Exit the program and free the memory.\n");
	printf("\n");
}
...
void uiAscendingByConcentrationForMedication(UI* ui)
{
	...
	{
		sortRepositoryByConcentration(dr);
		for (int i = 0; i < dr->length; i++)
			printMedication(&dr->elements[i]);
	}
...
void executeCommand(int cmdIndex, UI* ui)
{
	...
	if (cmdIndex == 4)
		uiSearchElement(ui);
	if (cmdIndex == 5)
		uiDescendingByPriceForMedication(ui);
	if (cmdIndex == 6)
		uiShortSupply(ui);
	if (cmdIndex == 7)
		uiCheapMedications(ui);
	if (cmdIndex == 8)
		uiAscendingByConcentrationForMedication(ui);
	if (cmdIndex == 9)
		uiUndo(ui);
	if (cmdIndex == 10)
		uiRedo(ui);
